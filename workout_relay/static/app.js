const translations = {
  en: {
    ...
    connectApprove: "Sign in to Workout Relay and approve the requested permissions. For activity access, Garmin may send Workout Relay GPS coordinates with your completed activity. Workout Relay removes coordinates and route data on its server before delivering the remaining metrics, including heart rate, to your assistant. Existing connections need fresh approval: remove and re-add the connector, then approve activities:read to ask about past runs.",
    activityKeyConsent: "Also allow completed activities and health/run metrics. Garmin may send GPS coordinates to Workout Relay; its server removes coordinates and route data before returning the filtered metrics through this key. Leave unchecked for plan-only access.",
    ...
    privacyKeptActivityIds: "If you grant activity access, Garmin may send Workout Relay an activity response containing GPS coordinates. Workout Relay processes that response in memory and removes coordinates and route data on its server before delivering filtered metrics to the AI. It does not save the activity response, measurements or coordinates in its application database. Activity names and times are delivered and may reveal location indirectly. To prevent requests for arbitrary Garmin activities, the database temporarily keeps the activity ID, your Garmin account name and when it was listed. Detail access expires after one hour and automatic cleanup removes the record; disconnecting Garmin or deleting your account also removes it.",
    privacyKeptSessions: "A hashed record of each signed-in browser session, and your language choice.",
    privacyPeriods: "Periods in force here: plans are deleted after {plans} days, a signed-in session lasts {session} days, a visit-only Garmin session expires after {visit} minutes, and an activity ID can authorize a detail request for {activities} hours.",
  ...
  fr: {
    ...
    connectApprove: "Connectez-vous à Workout Relay et approuvez les autorisations demandées. Pour l'accès aux activités, Garmin peut envoyer à Workout Relay les coordonnées GPS de votre activité terminée. Workout Relay supprime les coordonnées et le tracé sur son serveur avant de transmettre à votre assistant les autres mesures, dont la fréquence cardiaque. Pour une connexion existante, supprimez puis ajoutez à nouveau le connecteur et approuvez activities:read pour consulter vos courses passées.",
    activityKeyConsent: "Autoriser aussi les activités terminées et les mesures de santé/course. Garmin peut envoyer des coordonnées GPS à Workout Relay ; son serveur supprime les coordonnées et le tracé avant de renvoyer les mesures filtrées via cette clé. Laissez décoché pour limiter l'accès aux plans.",
    ...
    privacyKeptActivityIds: "Si vous autorisez l'accès aux activités, Garmin peut envoyer à Workout Relay une réponse contenant des coordonnées GPS. Workout Relay traite cette réponse en mémoire et supprime les coordonnées et le tracé sur son serveur avant de transmettre les mesures filtrées à l'IA. La réponse, les mesures et les coordonnées ne sont pas enregistrées dans la base de données de l'application. Le nom et l'heure de l'activité sont transmis et peuvent révéler indirectement un lieu. Pour empêcher toute demande d'activité Garmin arbitraire, la base conserve temporairement l'identifiant de l'activité, le nom du compte Garmin et la date de consultation. L'accès au détail expire après une heure et un nettoyage automatique supprime l'enregistrement ; la déconnexion de Garmin ou la suppression du compte le supprime également.",
    privacyKeptSessions: "Une empreinte de chaque session de navigateur connectée, et votre choix de langue.",
    privacyPeriods: "Durées appliquées ici : les plans sont supprimés après {plans} jours, une session connectée dure {session} jours, une session Garmin « cette visite uniquement » expire après {visit} minutes, et un identifiant d'activité peut autoriser une demande de détail pendant {activities} heures.",
